...
function add(page){
	if($('#new-' + window.page).val()) {
		$(this).after('<img class="loading" src="images/loading.gif" />');
		if(window.page == 'motifs') { $('#add-motifs').click(); }
		$.ajax({
			type: 'POST',
			data: { 'add' : $('#new-' + window.page).val(),
					'from-cat' : $('#from-cat').val(),
					'from-goal' : $('#from-goal').val(),
					'goal' : $('#from-goal').val(), 
					'deadline' : $('#deadline').val(), 
					'private' : $('#private').val() },
			url:  window.username + '/' + window.page,
			success: function(data){
				$('#content').html(data);
				$('#new-' + window.page).focus();
			}
		});
	}
}
...
function del(obj){
	if (confirm('Tem certeza que deseja apagar o item "' + $(obj).attr('confirm') + '"?')) {
		$.ajax({
			type: 'POST',
			data: { 'del' : $(obj).attr('value'), 'goal' : $(obj).attr('from-goal') },			
			url:  window.username + '/' + window.page,
			success: function(data){
				$('#' + obj.id).parent().fadeOut('slow',function(){ // Callback
					$('#content').html(data);
				});
			}
		});
...
function edit() {
	$('.edit').click(function(){
		if(page == 'motifs') {
			var old_value = $(this).next().text();
		} else {
			var old_value = $(this).next().next().text();
		}
		id = $(this).attr('value');
		$('#blur').show();
		$(this).parent().addClass('update').append('<div id="update"></div>');
		$('#update').html('<input id="updating" type="text" value="' + old_value + '" /><a href="javascript:;" id="button-update">Atualizar</a>');
		$('#updating').select();
	
		$('#button-update').click(function(){ 
			$.ajax({
				type: 'POST',
				data: { 'update' : $('#updating').attr('value'), 'id' : id },
				url:  window.username + '/' + window.page,
		...
		$('#blur').click(function(){
			$('#update').remove();
			$('li p').removeClass('update');
			$('#blur').hide();
		});
	
		$('#update').bind('keypress', function(event) { if(event.keyCode == 13) { $('#button-update').click(); } }); // Pressing enter
		$('#update').bind('keypress', function(event) { if(event.keyCode == 27) { $('#blur').click(); }	});  // Pressing esc
	});
}

function show_motifs(goal_id){
	$('#content').removeClass().fadeOut(300).addClass(page).slideDown(700);
	$.ajax({
		type: 'POST',
		data: { 'goal' : goal_id },
		url:  window.username + '/' + 'motifs',
		success: function(data){ $('#content').html(data); }
	});
}
...
function show_page(page){
	$('#link-' + page).click(function(){
		$('#content').removeClass().fadeOut(300).addClass(page).slideDown(700);
		$.ajax({
			url:  window.username + '/' + page,
			success: function(data){ $('#content').html(data); }
		});
	});
}

function show_tasks(goal_id){
	$('#content').removeClass().fadeOut(300).addClass('tasks').slideDown(700);
	$.ajax({
		type: 'POST',
		data: { 'goal' : goal_id },
		url:  window.username + '/' + 'tasks',
		success: function(data){ $('#content').html(data); }
	});
}
...
function toggle_done(){
	$('li input[type=checkbox]').click(function(){
		$(this).after('<img class="loading" src="images/loading.gif" />');
		if($(this).is(':checked') == true) { $(this).parent().addClass('strike'); }
		else { $(this).parent().removeClass('strike'); }

		$.ajax({
			type: 'POST',
			data: { 'done' : $(this).is(':checked'), 'id' : $(this).attr('name') },
			url: window.username + '/' + window.page,
			success: function(data){ $('#content').html(data); }
		});
	});
